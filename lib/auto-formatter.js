...
export default class AutoFormatter {
  ...
  formatIndents() {
    ...
    for (let row = 0; row <= this.buffer.getLastRow(); row++) {
      ...
      // 空行とコメント行は無視
      if (line.match(reEmpty)) continue;
      if (line.match(reComment)) continue;

      // 命題行の処理
      if (line.match(reStatement)) {
        let level = this.editor.indentationForBufferRow(row);
        // 命題のレベルが急に深くなる場合はレベルを+1までに制限する
        if (statementLevel + 1 < level) {
          level = statementLevel + 1;
          this.editor.setIndentationForBufferRow(row, level);
        }
        // 命題行のレベルを覚える
  ...
  assignUuids() {
    for (let row = 0; row <= this.buffer.getLastRow(); row++) {
      ...
      if (line.match(reUuidEmpty)) {
        line = line.replace(/uuid\s*$/, "uuid  " + nodeUuid.v4());
