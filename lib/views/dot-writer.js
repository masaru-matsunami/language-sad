  ...
  writeStatement(statement, deepLevel, forceId = null) {
    ...
    if (deepLevel > 1) {
      ...
      statement.children.forEach(child => {
        let childId = Config.treeGraph ? Helpers.shortUuid() : child.id;
        let color;
        ...
        if (Config.treeGraph) this.writeStatement(child, deepLevel, childId);
        else this.writeStatement(child, deepLevel);
      });
    }
  }
}
