...
export default class DotWriter {
  ...
  writeDotHeader() {
    ...
    this.writeLine('graph [size="100,100", rankdir=LR, splines=polyline, margin=0];');
  ...
  writeStatement(statement, opts) {
    ...
    if (statement.model === opts.rootStatement.model) {
      if (statement.label) label = statement.label;
    } else {
      // import したノード
      label = "-";
      if (statement.label) label = statement.label;
      else if (statement.uuid) label = statement.uuid.substring(0, 5);
    }
    ...
    if (opts.deepLevel > 1) {
      ...
      for (let idx = 0; idx < statement.children.length; idx++) {
        ...
        if (!Config.drawOutsideOfTree &&
          (exprChildId.startsWith("-") || exprChildId.length === 26)) {
