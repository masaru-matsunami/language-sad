...
export default class DotWriter {
  ...
  writeStatement(statement, forceId = null) {
    // Config.duplicateNode == false のときは、同じ命題を2回以上出力しない
    if (!Config.duplicateNode) {
    ...
    statement.children.forEach(child => {
      let childId = Config.duplicateNode ? Helpers.shortUuid() : child.id;
      let color;
      ...
      if (Config.duplicateNode) this.writeStatement(child, childId);
