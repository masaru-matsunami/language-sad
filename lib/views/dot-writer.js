...
export default class DotWriter {
  ...
  writeStatementRecursively(statement, deepLevel) {
    ...
    this.writeStatement(statement, deepLevel);
  ...
  writeStatement(statement, deepLevel, forceId = null) {
    // ネットワークグラフのときは、同じ命題を2回以上出力しない
    let graphType = atom.config.get('language-sad.graphType');
    ...
    let expr = (deepLevel === 1) ? statement.value : statement.expr;
    if (expr === undefined) expr = 'undef';
    this.writeLine(util.format(
      '"%s" [label="{<id>[%s]| %s |<expr>%s}", %s, shape=Mrecord];',
      id, escapeLabel(label), wrapText(statement.text, statementWidth),
      expr, colors));

    // 命題ノードのメモを描く
    if (atom.config.get('language-sad.showMemo')) {
      if (memos.length > 0) {
        let noteText = '';
        memos.forEach(memo => {
    ...
    if (deepLevel > 1) {
      ...
      for (let idx = 0; idx < statement.children.length; idx++) {
        ...
        let childDeepLevel = deepLevel - 1;
        if (!drawOutsideOfTree && exprChildId.startsWith("-")) childDeepLevel = 1;

        // 子ノードを描画
        let graphType = atom.config.get('language-sad.graphType');
        let childId = graphType === 'Tree' ? Helpers.shortUuid() : child.id;
        let color;
        ...
        if (graphType === 'Tree')
          this.writeStatement(child, childDeepLevel, childId);
        else
          this.writeStatement(child, childDeepLevel);
      }
    }
  }
}
