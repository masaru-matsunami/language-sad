...
export default class SadParser {
  ...
  parseLines(lines) {
    ...
    lines.forEach(line => {
      ...
      // import "path_to_sad_file"
      const reImport = /^import\s+"([^"]+\.sad)"/;
      let matches = line.match(reImport);
      if (matches !== null) {
        let import_path = matches[1];
        this.model.importFile(import_path);
        return;
      }

      // 命題行
      const reHead = /^\s*\[(\d+)\]\s*(.*)\s*$/;
      matches = line.match(reHead);
      if (matches !== null) {
        statement = new Statement();
        statement.model = this.model;
      ...
      if (matches !== null) {
        ...
        statement.childIds = matches[2].toLowerCase().replace(/\s/g, '').split(',');
